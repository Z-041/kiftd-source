	...
	if (check == "y") {
		...
		$.ajax({
			...
			success: function(result) {
				...
				rsaOaepEncrypt(publicKeyInfo.publicKey, loginInfo, function(encrypted) {
					if (encrypted) {
						...
						$("#alertbox").text("提示：登录失败，加密请求无法通过效验（可能是不安全的网络环境导致的）");
					}
				});
			},
			error: function() {
...
function doChangePassword() {
	...
	$.ajax({
		...
		success: function(result) {
			...
			rsaOaepEncrypt(changepwd_publicKeyInfo.publicKey, changePasswordInfo, function(encrypted) {
				if (encrypted) {
					sendChangePasswordInfo(encrypted);
				} else {
					showChangePasswordAlert("提示：密码修改失败，加密请求无法通过效验（可能是不安全的网络环境导致的）");
				}
			});
		},
		error: function() {
